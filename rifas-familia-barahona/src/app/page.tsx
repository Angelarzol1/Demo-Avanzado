...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {/* Hero Section */}
      <section className="pt-20 pb-16 px-6 bg-gradient-to-b from-gray-900 to-black">
        <div className="container mx-auto text-center">
          ...
          >
            ...
            <p className="text-xl md:text-2xl text-gray-300 mb-8">
              Rifas exclusivas con los mejores premios de Venezuela
            </p>
            <div className="flex flex-wrap justify-center gap-4 mb-12">
              <div className="bg-gray-800 rounded-full px-6 py-3 flex items-center space-x-2">
                <Shield className="h-5 w-5 text-green-400" />
                <span className="text-sm font-semibold">100% Seguro</span>
              </div>
              <div className="bg-gray-800 rounded-full px-6 py-3 flex items-center space-x-2">
                <Trophy className="h-5 w-5 text-yellow-400" />
                <span className="text-sm font-semibold">Premios Reales</span>
              </div>
              <div className="bg-gray-800 rounded-full px-6 py-3 flex items-center space-x-2">
                <Users className="h-5 w-5 text-blue-400" />
                <span className="text-sm font-semibold">Transparente</span>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              ...
              { icon: Users, label: 'Participantes', value: '2,547' },
              { icon: Award, label: 'Premios Entregados', value: '127' },
              { icon: BarChart3, label: 'Satisfacción', value: '98%' }
            ].map((stat, index) => (
              <motion.div
                key={stat.label}
                initial={{ opacity: 0, y: 20 }}
            ...
            {raffles.map((raffle, index) => {
              ...
              return (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: index * 0.1 }}
                  className="bg-gray-900 rounded-2xl overflow-hidden hover:transform hover:scale-105 transition-all duration-300 cursor-pointer"
                ...
                >
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-bold mb-2 line-clamp-2">{raffle.title}</h3>
                    ...
                    >
                      {isAvailable ? 'Participar Ahora' : 'Agotado'}
                    </button>
                  </div>
                </motion.div>
              );
